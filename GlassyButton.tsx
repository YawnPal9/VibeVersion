...
export function GlassyButton({ children, onClick }: GlassyButtonProps) {
  ...
  return (
    <button
      ...
      style={{
        ...
        marginLeft: "6px"  // spacing next to email input
      }}
    >
      {/* Glassy shine sweep */}
      <span
        style={{
          ...
          transform: hover ? "translateX(120%) skewX(-12deg)" : "translateX(-100%) skewX(-12deg)",
      ...
      <span
        style={{
          ...
          filter: "blur(3px)",
          animation: hover ? "orbit 2.2s linear infinite" : "none",
          pointerEvents: "none",
          opacity: 0.9
        }}
      />

      {/* Tiny flying pointer on hover */}
      {hover && (
        <span
          style={{
            position: "absolute",
      ...
      <style>{`
        ...
        @keyframes fly {
          ...
          50%  { transform: translate(20px,12px); opacity: 0.3; }
